...
const Cart = () => {
    ...
    const cartTotal = () => {
        return cart.reduce((acc, item) => acc + item.product.price * item.product.discount * item.quantity, 0)
    }
    /*  */
    const cartTotal2 = () => {
                    ...
                    <div className="grid my-5">
                                ...
                                <div className="item" key={item.product.id}>
                                    ...
                                    {/* <span className="price">${round(item.product.price * item.quantity, 2)}</span */}
                                            {item.product.discount === 1 ? (
                                                ...
                                                : (
                                                    <div>
                                                        <span className="price" style={{ marginRight: 15, color: "red", "textDecoration": "line-through" }}>${round(item.product.price * item.quantity, 2)}</span>
                                                        <span className="price">${round(item.product.price * item.product.discount * item.quantity, 2)}</span>
                                                    </div>
                                                )
                        ...
                        <div className="payment p-3">
                            ...
                            <div className="summary py-3 my-2">
                                {round(cartTotal2(), 2) !== round(cartTotal(), 2) ?
                                    (
                                     <div>  
                                        <div className="flex py-1">
                                            <span style={{ marginRight: 15, color: "red", "textDecoration": "line-through" }}>Subtotal:</span>
                                            <span className="price">${round(cartTotal2(), 2)}</span>
                                        </div>
                                        <div className="flex py-1">
                                    ...
                                    (
                                        <div className="flex py-1">
                                            <span style={{ marginRight: 15 }}>Subtotal:</span>
                                            <span className="price">${round(cartTotal2(), 2)}</span>
                                        </div>
